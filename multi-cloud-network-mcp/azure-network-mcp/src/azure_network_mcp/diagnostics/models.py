...
cannot reach a conclusion because required evidence is missing (an NSG
this server could not read, a peered VNet outside the analyzed resource
group, a UDR next hop this engine doesn't resolve) must say so, with
``limitations`` explaining what was missing -- silently omitting the
finding would look identical to "checked, found no issue," which is the
one thing this milestone's guardrails explicitly forbid ("claim certainty
...
class Evidence(BaseModel):
    ...
    (e.g. ``"effective_route:10.1.0.0/16"``,
    ``"nsg_rule:AllowHttps@nsg-web"``) so a reader can trace a conclusion
    back to a specific Azure API response field, never an inference
...
class RuleMetadata(BaseModel):
    ...
    prefixes: ``ROUTE-*`` (effective-route resolution), ``SEC-*`` (NSG
    evaluation), ``EXPOSE-*`` (internet exposure), ``CONSIST-*``
    (route/connection/resource-state consistency).
    """
