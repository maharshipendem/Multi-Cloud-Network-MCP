...
cannot reach a conclusion because required evidence is missing (a
hierarchical firewall policy this identity lacks visibility into, an NCC
route table this project can't see, an organization policy this server
has no API to read) must say so, with ``limitations`` explaining what was
missing -- silently omitting the finding would look identical to
"checked, found no issue," which this milestone's guardrails explicitly
forbid ("never claim certainty with missing organization policies").
"""
...
class Evidence(BaseModel):
    ...
    ``source`` identifies exactly which snapshot record the fact came
    from (e.g. ``"route:10.1.0.0/16"``,
    ``"firewall_rule:allow-internal@vpc-1"``) so a reader can trace a
    conclusion back to a specific GCP API response field, never an
...
class ReasoningStep(BaseModel):
    ...
    ``step`` is the step's 1-indexed position; steps are always emitted
    in the order they were actually evaluated, so replaying them
    reproduces the same reasoning a human would need to follow to reach
    the same conclusion by hand.
    """
...
class Finding(BaseModel):
    ...
    Every field this milestone's spec requires is present unconditionally
    ...
    nothing in this package (or anywhere in this repository) executes
    it. There is no "apply this fix" code path anywhere this field's
    value can reach.
    """
...
class RuleMetadata(BaseModel):
    ...
    prefixes: ``ROUTE-*`` (route priority/selection, CIDR overlap),
    ``PEER-*`` (VPC peering limitations), ``NCC-*`` (Network Connectivity
    Center propagation), ``FW-*`` (firewall ingress/egress, implied
    rules, hierarchical policy interaction), ``NAT-*`` (Cloud NAT
    egress), ``EXPOSE-*`` (public forwarding rules), ``HYBRID-*``
    (degraded VPN/BGP/Interconnect), ``DNS-*`` (DNS forwarding chains).
    """
