...
Every tool invocation is logged as a single structured record so the
output can be shipped as-is to Cloud Logging, Splunk, Datadog, or an ELK
stack. Credentials, access tokens, and full GCP API payloads are never
logged -- only normalized, operationally-relevant fields.
"""
